...
export default function Home() {
  ...
  const loadData = useCallback(async () => {
    const [itemsRes, catsRes, rulesRes, tagsRes] = await Promise.all([
      fetch('/api/items'),
  ...
  async function handleSubmit(message: string) {
    ...
    const optimisticItem: Item & { category?: Category } = {
      ...
      category_id: keywordMatch ?? null, project_id: null, completed_at: null,
    ...
    try {
      ...
      const saveRes = await fetch('/api/items', {
        ...
        body: JSON.stringify({
          ...
          project_id: routing.project_id, categorized_by: routing.method,
          habits: routing.habits, tags: routing.tags,
        }),
      })
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-2xl mx-auto px-4 py-4 space-y-3">
          ...
          {[...items].reverse().map((item) => (
            <MessageBubble key={item.id} item={item} categories={categories} allTags={tags}
              onReassign={handleReassign} onDelete={handleDelete} onTagClick={handleTagClick} onTagsUpdate={handleTagsUpdate} />
          ))}
          <div ref={bottomRef} />
        </div>
      </div>
