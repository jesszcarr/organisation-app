...
export default function SettingsPage() {
  ...
  async function addKeywordRule() {
    ...
    if (res.ok) { const rule = await res.json(); setKeywordRules((prev) => [...prev, rule]); setNewKeyword(''); toast.success('Rule added') }
        ...
        <div className="max-w-lg mx-auto px-4 py-6 space-y-10">
          ...
          <section className="space-y-4">
            ...
            <div className="space-y-1">
              {projects.map((proj) => (
                <div key={proj.id} className="flex items-center justify-between py-2.5 px-3 rounded-lg hover:bg-muted/50">
                  <div>
          ...
          <section className="space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              People and topics. The AI will only assign tags from this list — no auto-generated junk.
            </p>
              ...
              {habits.map((habit) => (
                <div key={habit.id} className="flex items-center justify-between py-2.5 px-3 rounded-lg hover:bg-muted/50">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {habit.track_type === 'binary' ? 'Yes/No' : habit.unit || 'numeric'}
                  </span>
                </div>
              ))}
            </div>
